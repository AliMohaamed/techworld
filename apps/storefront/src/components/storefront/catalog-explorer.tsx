...
export default function CatalogExplorer({
  ...
  const heading = lockedCategoryId
    ? (locale === 'en' ? category?.name_en : category?.name_ar) ?? t('locked.defaultHeading')
    : t('global.heading');
  
  const eyebrow = lockedCategoryId ? t('locked.eyebrow') : t('global.eyebrow');
  
  const description = lockedCategoryId
    ? (locale === 'en' ? category?.description_en : category?.description_en) ?? t('locked.defaultDescription')
    : t('global.description');

  return (
    <div className="min-h-screen bg-black px-4 pb-20 pt-8 md:px-8">
      <div className="mx-auto max-w-7xl space-y-10">
        <section className="overflow-hidden rounded-[32px] border border-white/10 bg-[radial-gradient(circle_at_top,#2b2b2b,transparent_50%),linear-gradient(180deg,#111111,#050505)] px-6 py-12 md:px-12 shadow-2xl">
          <div className="max-w-3xl space-y-6">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#ffc105]">
              {eyebrow}
            </p>
            <h1 className="font-space-grotesk text-4xl font-black uppercase tracking-tight text-white md:text-6xl md:leading-[1.1]">
              {heading}
            </h1>
            <p className="max-w-2xl text-sm leading-8 text-zinc-400 md:text-lg font-light">
              {description}
            </p>
          </div>
        </section>
        ...
        <div className="grid gap-8 lg:grid-cols-[340px_minmax(0,1fr)]">
          ...
          <div className="space-y-8">
            <div className="flex flex-wrap items-center justify-between gap-6 rounded-[28px] border border-white/10 bg-zinc-950/70 ltr:px-8 rtl:px-8 py-5 shadow-inner">
              <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-[0.35em] text-zinc-500">
                  {t('results.eyebrow')}
                </p>
                <p className="font-space-grotesk text-xl font-black uppercase tracking-tight text-white">
            ...
            {results.length === 0 && status === "Exhausted" ? (
              <div className="space-y-12">
                <div className="rounded-[32px] border border-dashed border-white/10 bg-zinc-950/60 px-8 py-16 text-center shadow-2xl">
                  <p className="text-[10px] font-black uppercase tracking-[0.35em] text-zinc-500">
                    {t('empty.eyebrow')}
                  </p>
                  <h2 className="mt-4 font-space-grotesk text-4xl font-black uppercase tracking-tight text-white">
                    {t('empty.title')}
                  </h2>
                  <p className="mx-auto mt-4 max-w-xl text-sm leading-8 text-zinc-400 font-light italic">
                    {t('empty.description')}
                  </p>
                </div>
