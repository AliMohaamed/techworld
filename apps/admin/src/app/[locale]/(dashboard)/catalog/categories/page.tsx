...
export default function AdminCategoriesPage() {
  ...
  const toggleStatus = async (id: Id<"categories">) => {
    ...
    try {
      ...
      const description = error instanceof Error ? error.message : "Status update failed.";
      ...
      <section className="rounded-3xl border border-white/5 bg-[#24201a] p-6">
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="rounded-full border border-white/5 px-3 py-1 text-xs text-zinc-300">
            {categories ? t('table.total', { count: categories.length }) : t('table.loading')}
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm text-zinc-300">
            <thead className="text-xs uppercase tracking-[0.2em] text-zinc-500">
              <tr>
                <th className="sticky left-0 bg-[#24201a] pb-3 pr-4 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.3)]">{t('table.columns.name')}</th>
                <th className="pb-3 pr-4">{t('table.columns.slug')}</th>
                <th className="pb-3 pr-4">{t('table.columns.products')}</th>
