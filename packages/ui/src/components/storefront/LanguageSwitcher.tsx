...
export function LanguageSwitcher({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full ltr:right-0 rtl:left-0 mt-2 flex w-36 flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/95 p-1 shadow-xl backdrop-blur-xl z-[100] animate-in fade-in slide-in-from-top-2">
          {languages.map((lang) => (
            <button
              key={lang.code}
